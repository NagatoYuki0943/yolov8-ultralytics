...
```sh
yolo task=detect mode=train imgsz=640 batch=-1 workers=16 epochs=300 optimizer=SGD lr0=0.01 cos_lr=True device=0 pretrained=weights/yolov8n.pt model=ultralytics/models/v8/yolov8n.yaml data=ultralytics/datasets/coco128.yaml
```

> `Adam`

```sh
yolo task=detect mode=train imgsz=640 batch=-1 workers=16 epochs=300 optimizer=AdamW lr0=0.001 cos_lr=True device=0 pretrained=weights/yolov8n.pt model=ultralytics/models/v8/yolov8n.yaml data=ultralytics/datasets/coco128.yaml
```
...
> 任务管理器中发现GPU的Copy在0和100之间严重波动，似乎训练完第一个epoch就好了，原因不明
